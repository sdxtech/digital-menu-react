    ...
    const results = await Promise.allSettled(
      this.uniqueRecipients(recipients).map((recipient) =>
        this.mail.enqueue({
          ...
          text: `Dear ${recipient.name.trim() || recipient.email},\n\n${message.message}\n\n${textDetails}\n\n${message.actionLabel}: ${url}\n\nNote : This is an automated notification from SPICES, please do not reply.`,
          html: `<div style="font-family:Arial,sans-serif;max-width:640px;color:#0f172a"><img src="${this.escapeHtml(logoUrl)}" alt="SPICES" width="140" style="display:block;width:140px;max-width:100%;height:auto;margin:0 0 20px;border:0" /><p>Dear ${this.escapeHtml(recipient.name.trim() || recipient.email)},</p><h2>${this.escapeHtml(message.title)}</h2><p>${this.escapeHtml(message.message)}</p><table>${htmlDetails}</table><p style="margin-top:24px"><a href="${this.escapeHtml(url)}" style="background:#2563eb;color:#fff;padding:10px 16px;text-decoration:none;border-radius:6px">${this.escapeHtml(message.actionLabel)}</a></p><p style="margin-top:24px;color:#64748b;font-size:12px">Note : This is an automated notification from SPICES, please do not reply.</p><img src="${this.escapeHtml(logoUrl)}" alt="SPICES" width="140" style="display:block;width:140px;max-width:100%;height:auto;margin:16px 0 0;border:0" /></div>`,
